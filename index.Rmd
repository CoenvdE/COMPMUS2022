...
My guitar teacher, me, and the sources mentioned earlier, think these artists really represent the genre in their time and think it is a good representation of what I want to research. I do think more many names could be added, such as **Bobby Hebbs**, but I don’t think this would be of use. 
...
comparing to the old generation.When comparing the low level Timbre components,
the genres did not differ that much, except for two components. This could be because all the artists represent
the soul genre, thus making similar music. Lastly, we saw that clustering the artists went fairly well, most artists
from the old and new generation where clustered together, except for **Erykah Badu** and **Ray Charles**. This could 
be because their music is more *timeless* than the music of other artists from the old generation, resulting in a
"clustering blend" between old and new generation in their case. From this can be concluded that the genre of soul
did change over generations, but it is still soul. There are no major changes, but there are changes. This means new
soul music is "innovative" compared to the older music.

What could be improved in **future research?**

In the portfolio, the old and new generation of soul artists exist of 6 (arbitrarily chosen) artists. While this gives some representation of how the generations compare, for further research it is adviced to use larger artist groups, with artists chosen carefully. Furthermore, the outliers in the generations are not taken out in this research, this is because Spotify keeps putting them in and taking them out. This means sometimes the outliers are in the data and other times they are not. To improve this, the outliers should be removed at the time they are both in there, after which teh data is saved to work with in the future. Another point is that the comparison of an original with a cover, only consists of 1 cover. This is a very small number of comparisons, and needs to be bigger in future research.
...
To understand how the structure of a track works, a track of the new generation is visualized with *Self Similarity matrices (SSM)*. The track is made by 2 artist from the new generation, making it more representable for the genre than other songs. In the Self-Similarity matrices, the track *Losing my way* from *Tom Misch & FKJ* is analysed.

First we analyse via a **Timbre SSM**. **Timbre** can be described as the feel of a track, and what different instruments are used, but the term is hard to describe. As the **Timbre SSM shows**, there are different sections in the song that repeated. The sections are as A - B - C - A - (B+C) - D. 

The **Chroma SSM** shows how **pitch** characteristics of a song relate to eachother As we can see in the chroma SS matrix, different segments of the song have different pitch characteristics. Light means the pitch characteristics are way different there then in the rest of the track. This is the case for the intro, bridge and outro of the tra. The rest of the pattern follows the sections quite well, as explained above. The colors in the rest of the matrix are darker, even though the pitch characteristics are quite different during the rest of the song. This is because compared to the intro and outro, the pitch characteristics during the rest of the track are quite similar.
