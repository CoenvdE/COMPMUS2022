...
hc_fkj <- hclust(fkj_dist, method = "complete")
dend <- as.dendrogram(hc_fkj)
par(mfrow = c(1,2))
dend <- dend %>%
  color_branches(k = 10) %>%
...
#   dendro_data() %>%
#   ggdendrogram() + theme_ilo() + theme_tufte()
...
For my project, I wanted to research soul music. I am a musician myself, I make soul/jazz/funk/RnB. I am curious how the soul music I listen and make everyday came to be and how the genre may differ between earlier and later generations. This is why I decided to see how big soul artists through the years compare to eachother. To decide what artists I should include, I made a mix of artists that inspire me, artists that have meant a lot for the genre in the perspective of my guitar teacher (who finished his jazz conservatory master) and artists that made the top according to the internet (see source below). These are the artists per generation:
...
What is interesting to see is that there is one **outlier**, a song of *Erykah Badu*. This one is called *"Hi"*, which is not a full song, but definitely music, it is in the *"This is Erykah Badu"* playlist. This song has **no  valence and tempo** stats, they are set to 0. Another interesting **outlier** is that a song of *Bill Withers, "I Don't Want You On My Mind"*, which has a **very high tempo** (240-250).
...
the **outlier** song of *Bill Withers, "I Don't Want You On My Mind"* has a which **very high tempo** (240-250). I wanted to see how this can be possible, since it is extraordinary to have a track with such high bpm.
...
Another interesting point to see if (and how) the generations differ from eachother, is to look at the cover of a song from the old generation, by an artist of the new generation.
...
### appendix: **Self-Similarity Matrices** for a song of **Tom Misch & FKJ**
