...
### Overview of the generations **valence and tempo**
...
ggplot(all, aes(x=tempo , y= playlist_name, size = valence, color = playlist_name)) +
  ...
  labs(title = "Old Generation", subtitle = "Valence and tempo per artist playlist", colour = "Playlist name",
       x = "Tempo", y = "Playlist name", size = "Valence") + theme_tufte() + theme_ilo() + facet_grid( facets = . ~ generation)
...
What is interesting to see is that there is one **outlier**, a song of *Erykah Badu*. This one is called *"Hi"*, which is not a full song, but definitely music, it is in the *"This is Erykah Badu"* playlist. This song has **no data**, valence and tempo are set to 0. Another interesting point is that 1 song of *Bill Withers*,*Dont want you on my mind*, has a **very high tempo** (240-250). There is also an **outlier**, like *"Hi"*, in the *This is Mahalia* playlist, Spotify has taken it out during the time I worked on the project. I Think this is a good decision. since it was a *track chat*, a discussion about a track she made, and not an actual song. 
...
### comparing the generations **valence and tempo**
...
### comparing the generations **danceability and energy**
...
As can be seen in the visualization, the *new generation* generally makes songs that have **higher danceability**, but **lower energy** than the *old generation*. I think it is interesting that the **tempo** goes down (as can be seen in the comparison of **tempo and valence**), but the **danceability** goes up. This could imply that songs are getting **groovier**.
...
I think this song is very interesting to see in the chromogram, as you can "see the transposing happen".

### comparing *say a little prayer* from the old generation with a cover of the new generation with **Dynamic time wraping**
...
In the dynamic time warping graph, *Say a little prayer* by *Aretha Franklin* is compared to the *cover* of *Lianne de Havas*. As we can see in the visualization WHAT CAN WE CONCLUDE???
